...
FIXME: add an extension called jooqConfig. this would allow you to override.
FIXME: jooq extension just allows for one jooq config
FIXME: additional jooq configs would have to be defined as tasks
FIXME: generate one gradle plugin per version per dist type
FIXME: the task can lookup the jooqConfig -- if not found, the use defaults
