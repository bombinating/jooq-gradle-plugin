...
Tested with jOOQ versions 3.10, 3.11 and 3.12 (Open Source and Pro editions) on Java 1.8, 11 and 13.
...
If the `edition` or `version` is not specified, they default to `OpenSource` and `3.12.2`, respectively.
...
|`version`|jOOQ version to use|`3.12.2`
...
dependencies {
    compile(group = "org.jooq", name = "jooq", version = "3.12.2")
...
* the jOOQ version is specified (`3.12.2`)
