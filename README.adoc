...
* In this plugin, the `jooq` extension defines a single task jOOQ code generation task called `jooq`. Other jOOQ code generation tasks are explicitly defined using the Gradle task mechanism rather than being implicitly created in the `jooq` extension block.
